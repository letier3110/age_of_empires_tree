...
class Tree {
  constructor() {
    ...
    this.height = Math.max(window.innerHeight - 80, 1200)
    this.width = 0
    this.padding = 20
...
function unique(ids, monk_prefix) {
  ...
  SVG.get('unit_' + formatId(UNIQUE_UNIT) + '_text').text(
    formatName(data.strings[data.data.units[ids[0]].LanguageNameId])
  )
  SVG.get('unit_' + formatId(UNIQUE_UNIT) + '_overlay').data({
    name: data.strings[data.data.units[ids[0]].LanguageNameId],
    id: 'unit_' + ids[0],
  })
  SVG.get('unit_' + formatId(ELITE_UNIQUE_UNIT) + '_text').text(
    formatName(data.strings[data.data.units[ids[1]].LanguageNameId])
  )
  SVG.get('unit_' + formatId(ELITE_UNIQUE_UNIT) + '_overlay').data({
    name: data.strings[data.data.units[ids[1]].LanguageNameId],
    id: 'unit_' + ids[1],
  })
  SVG.get('tech_' + formatId(UNIQUE_TECH_1) + '_text').text(
    formatName(data.strings[data.data.techs[ids[2]].LanguageNameId])
  )
  SVG.get('tech_' + formatId(UNIQUE_TECH_1) + '_overlay').data({
    name: data.strings[data.data.techs[ids[2]].LanguageNameId],
    id: 'tech_' + ids[2],
  })
  SVG.get('tech_' + formatId(UNIQUE_TECH_2) + '_text').text(
    formatName(data.strings[data.data.techs[ids[3]].LanguageNameId])
  )
  SVG.get('tech_' + formatId(UNIQUE_TECH_2) + '_overlay').data({
    name: data.strings[data.data.techs[ids[3]].LanguageNameId],
    id: 'tech_' + ids[3],
  })
  SVG.get('unit_' + formatId(UNIQUE_UNIT) + '_img').load('assets/Units/' + formatId(ids[0]) + '.png')
  SVG.get('unit_' + formatId(ELITE_UNIQUE_UNIT) + '_img').load('assets/Units/' + formatId(ids[1]) + '.png')
  SVG.get('unit_' + formatId(MONK) + '_img').load('assets/Units/' + monk_prefix + '125.png')
}
